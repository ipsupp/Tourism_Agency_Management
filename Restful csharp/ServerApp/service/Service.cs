    ...
    public class Service
    {
        ...
        public void HandleMessage(HttpListenerContext context)
        {
            ...
            if (request.HttpMethod == "POST")
            {
                ...
                try
                {
                    trip_repository.add(JsonConvert.DeserializeObject<Trip>(body));
                ...
                catch(Exception e)
                {
                    ...
                    writer.WriteLine("406");
                    writer.Flush();
                }
            }
            if (request.HttpMethod == "GET")
            {
                //Cautare (dupa id si toate trips)
                try
                {
                    string json;
                    if (body == "")
                        json = JsonConvert.SerializeObject(trip_repository.findAll());
                    else
                        json = JsonConvert.SerializeObject(trip_repository.find(body));
                    writer.WriteLine("200 "+json);
                    writer.Flush();
                }
                catch(Exception e)
                {
            ...
            if (request.HttpMethod == "PATCH")
            {
                ...
                try
                {
                    ...
                    val = body.Split(",");
                    trip_repository.update(val[0], Convert.ToInt32(val[1]));
